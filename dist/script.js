$(function(){
  ...
  const globalState = {
    ...
    // apps라는 집단에 묶여있음
    // 각 어플별로 widgetFull, app 이라는 type으로 분류됨
    // icono는 어플 아이콘으로 firebasestorage에서 가져옴
    // nombre 이름 변경하면 다른 기능 고장나기도함 -> 어떻게 엮여있나 찾아봐야됨
    ...
    bateriaBaja: false,   // 배터리 부족 경고 표시 flag -> true면 경고표시 뜸
    draggScreen: false    // 
  }
  //EXtended Functions
  $.fn.extend({
...
<div class="manecilla segundos" style="transform: rotate(${transformSegundos});"><div class="barra"></div></div>
    ...
    fecha: function (config) {
      ...
      let dia = globalState.dateTime.dias[config.fecha.getDay()];
      let mes = globalState.dateTime.meses[config.fecha.getMonth()];
      this.text(`${mes} ${hoy}일 ${config.diaCompleto ? dia : dia.substring(0, 3)}`); // 요일, 
      return this;
    },
  })
...
`);
  ...
  // 카메라 기능
  // 원래는 저장된 비디오 출력하는거였는데
  // 웹캠화면 출력되도록 수정함
  // <video id="webcamVideo" autoplay="true"></video>
  function camara(){
    if (!$('.camaraApp').length) {
      $('.mainScreen').append(`
...
<path d="M62 38l-7.259-9.86L46 36"></path>
  ...
  function renderizarUI(){
    ...
    if ($('.wrapperApps .app[data-app="widgetFullCalendario"]').length) {
      ...
      $('.wrapperApps .app[data-app="widgetFullCalendario"] .icono').append('<div class="eventos"><p>오늘은 더이상 일정이 없습니다.</p></div><div class="calendarioWrapper"></div>');
...
<div class="menuOption eliminar">앱 제거
<div class="icono">
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
...
<div class="menuOption shaking">홈 화면 편집
<div class="icono">
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
...
`);
  ...
  // 화면 아래 꾹누르면 어플 위치 변경 가능하고, 어플 흔들리는 함수
  ...
  $('body').on('click', '.fixedMenuFixedApp .menuOption.eliminar', function () {
    ...
    alertaiOS({
      encabezado: `${idApp !== undefined ? globalState.apps[idApp].nombre : 'app'} 을(를) 홈 화면에서 제거하거나 완전히 삭제하겠습니까?`,
      mensaje: '앱을 드래그하면 모든 데이터를 유지하면서 홈 화면에서 앱이 제거됩니다.',
      acciones: [
        {
          texto: '앱 제거',
          warning: true,
